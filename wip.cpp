...
#define RootDirSizeMSB 0x80AC		// +1 to grab 2nd byte , Also written as both MSB and LSB in the PVD, A lot of CD games report 2048bytes (full track) for root dir listing size for some reason.
#define DiscSectorSize 2048


string RegionStringGlobal;


char FETCHED_ID[10];
std::string CapturedID[10];
std:: string DriveLetter;
std::string TempWorkPath;
long long PositionIndicator;
int GameIDloop = 0;
...
string ID_String_conversion(char* x) {
	...
	// Get RootDir Location
		fstream file("SLUS_202.26.iso", ios::in | ios::binary);
			int RootDirLoc0, RootDirLoc1, RootDirConv;
				char x;
					file.seekg(GrabDirRecordMSB, ios::beg);
						file.read((&x), 1);
							RootDirLoc0 = static_cast<int>(x);
								file.seekg(GrabDirRecordMSB+0x01, ios::beg);
									file.read((&x), 1);
	...
	// Store RootDir in RAM
		...
		for (int i = 0; i < 1024; i++) {
			file.seekg(RootDirectoryLocation + i, ios::beg);
			file.read((&x), 1);
	...
	// Find HEX "53 59 53 54 45 4d 2e 43 4e 46 3b 31" / " SYSTEM.CNF;1 " in 
