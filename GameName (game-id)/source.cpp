...
int main()
{
	...
	for (fileindex = 0; fileindex < amount_of_files; fileindex++)
	{
		...
		if (!endOfFileReached) {
			...
			std::cout << "Game Title/ ID: " << Find_Game_ID << std::endl;

			// Find the position of the first space after the GameID
			size_t first_space_pos = Find_Game_ID.find(' ');

			if (first_space_pos != std::string::npos) {
				// Extract the GameID (everything before the first space)
				std::string gameID = Find_Game_ID.substr(0, first_space_pos);

				// Extract the game title (everything after the first space)
				std::string gameTitle = Find_Game_ID.substr(first_space_pos + 1);

				// Remove leading/trailing whitespace or null characters (optional)
				gameTitle.erase(0, gameTitle.find_first_not_of(" \n\r\t"));  // Trim leading whitespace
				gameTitle.erase(gameTitle.find_last_not_of(" \n\r\t") + 1);  // Trim trailing whitespace

				// Create the new filename by appending GameID inside parentheses
				std::string newfilename = gameTitle + " (" + gameID + ").iso";

				std::cout << "New file name: " << newfilename << std::endl;

				std::cout << "GameID for title rename found @ line: 0x " << linecounter << std::endl;
				...
				if (successful_rename == 0) {
					std::cout << "Game renamed successfully!" << std::endl;
				}
				else {
					std::cout << "Error renaming, game open elsewhere or already named as per list?" << std::endl;
				}

				std::cout << std::endl << std::endl << std::endl;
			}
			else {
				std::cout << "Error: GameID not found or file format is incorrect!" << std::endl;
			}
		}
		if (!endOfFileReached) {
